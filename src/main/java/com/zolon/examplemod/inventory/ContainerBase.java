...
public class ContainerBase extends Container {
	...
	// Stores a reference to the tile entity instance for later use
		...
		public ContainerBase(InventoryPlayer invPlayer, TileEntityInventoryBase TileEntityInventoryBase) {
			this.TileEntityInventoryBase = TileEntityInventoryBase;
	    ...
	    final int SLOT_Y_SPACING = 18;
			...
			if (TE_INVENTORY_SLOT_COUNT != TileEntityInventoryBase.getSizeInventory()) {
				System.err.println("Mismatched slot count in ContainerBasic(" + TE_INVENTORY_SLOT_COUNT
													  + ") and TileInventory (" + TileEntityInventoryBase.getSizeInventory()+")");
			}
			...
			for (int x = 0; x < TE_INVENTORY_SLOT_COUNT; x++) {
				int slotNumber = x;
				addSlotToContainer(new Slot(TileEntityInventoryBase, slotNumber, TILE_INVENTORY_XPOS + SLOT_X_SPACING * x, TILE_INVENTORY_YPOS));
			}
		}
		...
		{
			return TileEntityInventoryBase.isUsableByPlayer(player);
		}
		...
		public void onContainerClosed(EntityPlayer playerIn)
		{
			...
			this.TileEntityInventoryBase.closeInventory(playerIn);
	}
